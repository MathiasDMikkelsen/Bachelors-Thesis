...
# =============================================================================
alpha = 0.7; beta = 0.2; gamma = 0.2; t = 24.0; d0 = 0.5
...
# =============================================================================
tau_z_baseline = 1.0
sol_b1, res_b1, c1 = solve(tau_w, tau_z_baseline, g)
sol_b2, res_b2, c2 = solve(tau_w_alt, tau_z_baseline, g)
if not (c1 and c2):
    raise RuntimeError("Baseline did not converge.")
# unpack
w_b1, p_d_b1 = res_b1["w"], res_b1["p_d"]
l_agents_b1, c_b1, d_b1 = res_b1["l_agents"], res_b1["c_agents"], res_b1["d_agents"]
w_b2, p_d_b2 = res_b2["w"], res_b2["p_d"]
l_agents_b2, c_b2, d_b2 = res_b2["l_agents"], res_b2["c_agents"], res_b2["d_agents"]
...
base_inc1 = np.array([phi[i]*w_b1*(1-tau_w[i])*(t-l_agents_b1[i]) + res_b1["l"] for i in range(n)])
base_exp2 = np.array([E_star(p_d_b2, U_b2[i]) for i in range(n)])
base_inc2 = np.array([phi[i]*w_b2*(1-tau_w_alt[i])*(t-l_agents_b2[i]) + res_b2["l"] for i in range(n)])
...
# =============================================================================
# 4. Plotting: 3 panels + new log‐utility panel + legend row
# =============================================================================
...
gs = gridspec.GridSpec(3, 2, height_ratios=[1,1,0.2], hspace=0.40, wspace=0.05)

ax0 = fig.add_subplot(gs[0,0])  # CV rel income
ax1 = fig.add_subplot(gs[0,1])  # income change
ax2 = fig.add_subplot(gs[1,0])  # exp utility change
ax3 = fig.add_subplot(gs[1,1])  # log utility change
...
ax0.set(xlabel=r'Environmental tax ($\tau_z$)', title='CV relative to disposable income')
ax0.tick_params(labelsize=12); ax0.set_xlim(tau_z_values[0], tau_z_values[-1])
ax0.yaxis.set_label_coords(-0.15,0.5); ax0.set_box_aspect(1)
...
ax1.set(xlabel=r'Environmental tax ($\tau_z$)', title='Total disposable income change')
ax1.tick_params(labelsize=12); ax1.set_xlim(tau_z_values[0], tau_z_values[-1])
ax1.yaxis.set_label_coords(-0.15,0.5); ax1.set_box_aspect(1)
...
ax2.set(xlabel=r'Environmental tax ($\tau_z$)', title='Change in blue utility')
ax2.tick_params(labelsize=12); ax2.set_xlim(tau_z_values[0], tau_z_values[-1])
ax2.yaxis.set_label_coords(-0.15,0.5); ax2.set_box_aspect(1)
...
ax3.set(xlabel=r'Environmental tax ($\tau_z$)', title='Change in log blue utility')
ax3.tick_params(labelsize=12); ax3.set_xlim(tau_z_values[0], tau_z_values[-1])
ax3.yaxis.set_label_coords(-0.15,0.5); ax3.set_box_aspect(1)
ax3.grid(True, color='grey', linestyle='--', linewidth=0.3, alpha=0.5)

# Legend row
ax4.axis('off')
handles = [Line2D([0],[0],color=colors[i],lw=lw,label=rf'Household ${i+1}$') for i in range(n)]
ax4.legend(handles=handles, loc='center', ncol=n, frameon=False, fontsize=10)
