...
def solve(tau_w, tau_z, g, r, d0):
    """
    solve the ge given:
      - tau_w: array of length n (labor-income tax rates)
      - tau_z: scalar     (environmental tax rate)
      - g:     scalar     (government spending)
      - r:     scalar     (ces exponent)
      - d0:    scalar     (baseline consumption of good d)
    returns: (solution vector, results dict, converged flag)
    """
    ...
    def system_eqns(y):
        ...
        eq7 = n*l - (np.sum(tau_w * w * phi * (t - l_agents)) +
                     tau_z*(z_c + z_d) - g)
    ...
    # recompute aggregates & utilities
    f_c = (epsilon_c * t_c**r + (1 - epsilon_c) * z_c**r)**(1/r)
    f_d = (epsilon_d * t_d**r + (1 - epsilon_d) * z_d**r)**(1/r)
...
# —————————————————————————————————————————————————————————————————————————————
# automatic test on import
# —————————————————————————————————————————————————————————————————————————————
tau_w = np.array([0.015, 0.072, 0.115, 0.156, 0.24])
tau_z = 4.0
g     = 5.0
r     = -1.0  # ces exponent
d0    = 0.5   # baseline durables demand
