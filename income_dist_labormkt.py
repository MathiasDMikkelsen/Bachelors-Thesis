...
phi_d = np.array([0.05*2, 0.45*2])   # Dirty sector households (sum = 1.0)
...
n = len(phi)
n_d = len(phi_d) # Number of household types in dirty sector
n_c = len(phi_c) # Number of household types in clean sector
...
try:
    ...
    for i in range(n):
        print(f"household {i+1} (phi={phi[i]:.3f}): c={results['c_agents'][i]:.4f}, d={results['d_agents'][i]:.4f}, leisure={results['l_agents'][i]:.4f}, labor={results['labor_supply_agents'][i]:.4f}")
    ...
    for i in range(n):
        print(f"household {i+1}: error = {results['budget_errors'][i]:.4e}")
...
# --- Make sure it's placed INSIDE the 'try' block ---
# --- Place it before the 'except Exception as e:' line ---
...
if converged:
    ...
    # Retrieve necessary variables from results or global scope
    w_c = results['w_c']
    w_d = results['w_d']
    l = results['l']
    labor_supply_agents = results['labor_supply_agents'] # Use labor supply time
    # phi, tau_w, t, n_d, n_c are parameters defined earlier

    # Construct the wage vector applicable to each household based on their sector
    wage = np.concatenate([w_d * np.ones(n_d), w_c * np.ones(n_c)])

    # Calculate Net Labor Income for each household type
    # Income = productivity * sector_wage * (1 - tax_rate) * labor_time
    net_labor_income = phi * wage * (1 - tau_w) * labor_supply_agents

    # Calculate Total Disposable Income for each household type
    # Total Income = Net Labor Income + Lump-sum Transfer
    total_disposable_income = net_labor_income + l

    print("\nHousehold Disposable Income Breakdown:")
    # Adjusted formatting width for the extra columns
    print("-" * 80)
    print(f"{'Household':<10} {'Sector':<8} {'Phi':<10} {'Wage':<10} {'Net Labor Inc':<15} {'Transfer (l)':<15} {'Total Income':<15}")
    print("-" * 80)
    for i in range(n):
        sector = "Dirty" if i < n_d else "Clean"
        sector_wage = wage[i] # Will be w_d for i < n_d, w_c otherwise
        print(f"{i+1:<10} {sector:<8} {phi[i]:<10.3f} {sector_wage:<10.4f} {net_labor_income[i]:<15.4f} {l:<15.4f} {total_disposable_income[i]:<15.4f}")
    print("-" * 80)
    ...
    print("-" * 35)
    print(f"{'Household':<10} {'Share (%)':<15}")
    print("-" * 35)
    for i in range(n):
        print(f"{i+1:<10} {income_shares[i]*100:<15.2f}%")
    print("-" * 35)

    # --- Simple Inequality Measures ---
    max_income = np.max(total_disposable_income)
    min_income = np.min(total_disposable_income)

    print("\nIncome Inequality Measures:")
    if min_income > 1e-9: # Avoid division by zero
         print(f"Max/Min Income Ratio: {max_income / min_income:.4f}")
    else:
         print("Max/Min Income Ratio: Minimum income is zero or near zero.")
    print(f"Income Range (Max - Min): {max_income - min_income:.4f}")

# Note: The existing 'if not converged:' block already handles printing messages
# if the solver fails, so we don't strictly need an 'else' here,
# assuming this code is placed *after* the initial convergence check message.
# If you want a specific message *about* income analysis failure, you could add:
# else:
#     print("\nSolver did not converge. Income analysis cannot be performed reliably.")
