...
# ensure we can import our extended solvers
...
from a_solvers.inner_solver_ext import n, alpha, beta, gamma, d0, phi, varphi
...
# helper: optimize only τ_z for fixed τ_w
...
for ax, pa in zip(axes.flat, p_a_list):
    ...
    # fixed optimal@ξ=0.1
    for xi in xi_values:
        tz, _ = maximize_welfare_fixed_w(G_value, xi, fixed_tau_w_optimal_xi01, pa, 2.0)
...
plt.savefig("d_extension/b_robustness_p_a.pdf", bbox_inches='tight')

# --------------------------------------------------------------------
# 2) Robustness over varsigma
# --------------------------------------------------------------------
...
for ax, vs in zip(axes2.flat, varsigma_list):
    ...
    for xi in xi_values:
        ...
        tz_pre.append(tz if tz is not None else np.nan)
    for xi in xi_values:
        tz, _ = maximize_welfare_fixed_w(G_value, xi, fixed_tau_w_optimal_xi01, 5.0, vs)
        tz_fix.append(tz if tz is not None else np.nan)
...
plt.tight_layout()
plt.savefig("e_appendix/c_robustness_varsigma.pdf", bbox_inches='tight')