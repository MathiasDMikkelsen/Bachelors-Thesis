...
#     (pre‐existing τ_w vs. optimal τ_w at ξ=0.1)  ← flipped by multiplying by –1
...
# ─── Compute aggregate welfare differences over τ_z_values ────────────────────
...
for j, tz in enumerate(tau_z_values):
    ...
    if not convp:
        raise RuntimeError(f"Model did not converge at τ_z = {tz:.3f}")
...
poll_slope  = poll_spline.derivative()(tau_z_smooth)  # d/dτ_z (z_c+z_d)
...
plt.plot(
    ...
    label=r'Blue loss (pre-existing $\tau_w$)'
)
plt.plot(
    tau_z_smooth, -slope2,
    color='tab:green', linestyle=':', linewidth=2,
    label=r'Blue loss (optimal $\tau_w$ at $\xi=0.1$)'
)

# (2) Flipped slopes of penalty term d/dτ_z[5·ξ·(z_c+z_d)] for three ξ
xis           = [0.1, 0.55, 1.0]
maroon_shades = ['#f2dede', '#c9302c', '#7b1113']  # light → medium → dark
for xi_val, col in zip(xis, maroon_shades):
    plt.plot(
        tau_z_smooth,
        -5 * xi_val * poll_slope,  # now flipped
        linewidth=2,
        color=col,
        label=rf'Green benefit ($\xi={xi_val}$)'
    )
...
plt.savefig("c_opttax/g_tradeoff.pdf", bbox_inches='tight')
