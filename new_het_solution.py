...
def households_focs_array(c, d, ell, p_c, p_d, w,                           # Defines a Numba-jitted function for the household FOCs
    ...
    for i in range(n):                                                     # Loops over each household
        if (c[i] <= 0.0) or (d[i] <= d0) or (ell[i] <= 0.0) or (ell[i] >= 1.0):
            ...
            foc_ell = gamma * (ell[i]**(gamma - 1.0)) - mult[i] * w * psi[i] # FOC for leisure
...
@nb.njit
def households_budget_array(c, d, ell, p_c, p_d, w):                       # Numba-jitted function for household budget constraints
    """
    For each household, the budget constraint is:
      p_c * c_i + p_d * d_i = w * (1 - ell_i)
    Returns a vector of length n.
    """
    n = c.shape[0]                                                         # Number of households
    res = np.empty(n)                                                      # Pre-allocates an array for budget constraints
    for i in range(n):                                                     # Loops over each household
        res[i] = p_c * c[i] + p_d * d[i] - w * (1.0 - ell[i])               # p_c c_i + p_d d_i - w * (1 - ell_i)
...
def main_solve_for_tau(tau_z=4, n=5):                                     # Defines the main function to solve the system
    ...
    # Define a vector of household productivities (psi) that sum to 1
    psi_temp = np.array([0.1, 0.15, 0.2, 0.25, 0.3])
    psi = psi_temp / psi_temp.sum()                                       # Normalize so sum of psi is 1
    ...
    # Solve
    sol = root(lambda x: full_system(x, params, n), u0, method='lm', tol=1e-8)  # Calls the solver
    final_res = full_system(sol.x, params, n)                              # Final residual array
